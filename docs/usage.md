...
Observations, including failed-extraction source responses, are stored locally under `~/.cache/finviz-skill/observations.sqlite3` unless `--store` or `FINVIZ_STORE` selects another location. `read ID` shows a saved observation again with the original command's selectors, and `read ID --raw --chars START-END` returns its response text; neither contacts Finviz. Each command answers with one screenful by default and reports what it received, matched and showed under `coverage`; a result over `--max-chars` shows the records that fit, reports `partial` and names a `continuation` command that reads the rest of the same selection. Retain a store while using its result IDs and delete it when the material is no longer needed.
