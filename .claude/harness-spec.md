...
- 단계별 codex 검토의 P1/P2를 재현·수정했다. 최종 본문·도움말·스키마 사용성 검토에서 발견한 옛 fetch 안내도 profile/about으로 수정했다. 별도 모델 세션 e2e는 추가 비용 동의를 받지 않아 실행하지 않았다.
- Graphify 코드 그래프를 세 차례 갱신했다. 마지막 결과는 558노드·1,315간선·28커뮤니티다. 마지막 자동 명명은 Claude 세션 한도로 실패하여 기본 이름을 사용하며, AST 그래프 자체는 갱신됐다.
