...
naver-blog는 [승인 계획](plans/naver-blog%20스킬%20구현%20계획.md)의 D1–D12·P0–P6와 공개 seam 다섯을 따른다. Aside u0 실계정 로그인 세션을 쓰는 독립 읽기 전용 스킬이며, 네이버에는 회전하는 토큰이 없어 헤더 계약이 호스트별 referer 하나뿐이다. 대신 서버가 자기 총수를 모르는 표면이 많아 종료 판정·1,000건 상한·페이지 정체를 CLI 경계가 책임지고, 글 본문은 유일하게 드리프트하는 표면이라 커버리지 회계가 출력에 드러난다. 영어 본문/도움말/주석과 한국어 트리거, 사용자 스코프 심볼릭 링크로 배포한다. Claude 헤드리스 e2e는 실행하지 않는다.
...
Threads 추가 검증은 [통합 계획의 구현 기록](plans/threads%20스킬%20구현%20계획.md)에 명령·수치·실패 후 수정·라이브 미확인 범위를 함께 기록한다. 최종 통합 CI는 Python 687개·JS 61개 통과(Threads 오프라인 92개·JS 16개 포함), 하네스 감사는 skill 4개·드리프트 0·오류/경고 0이다. PR #4는 3fa0810으로 스쿼시 머지됐다.

naver-blog 검증: 오프라인 335개·JavaScript 9개가 통과하고, 전체 레포는 1,005개 통과·39개 deselected다. Ruff·PII 게이트·`validate_harness.py`(오류 0·경고 0)·`audit_harness.py`(skill 5개·드리프트 0) 통과. 실계정 라이브 15개는 누적 30요청 가드 안에서 통과했고, doctor가 `chunghun1`을 보고하며 `BuddyPostList`가 `countPerPage` 3과 30에 같은 집합을 돌려줘 F14가 회귀하지 않았음을 확인했다.

단계별 codex 리뷰 6회(P0·P1·P2·P3·P4·P5 문서)에서 제기된 지적을 전부 재현 테스트와 함께 반영했다. P3·P4 리뷰는 각각 "승인하기 어렵다"·"승인 보류"로 판정했고, 그 근거가 맞았다 — 정상 HTML에서 본문 순서가 뒤집히고(`앞<b>중간</b>뒤`→`앞뒤중간`), 닫히지 않은 `<p>`에서 RecursionError가 나고, `data-module`의 실제 구조(`{type,data}`)를 읽지 못하고, `post --comments`가 차단을 "댓글 없는 글"로 바꾸고 있었다. 소스 접근 없이 수행한 사용성 검토 V1–V5는 다섯 시나리오를 모두 수행하고 근접 오발 셋(네이버 카페·티스토리·네이버 뉴스)을 정확히 거절했으며, 그 과정에서 네이버가 최상위 댓글의 `parentCommentNo`를 자기 자신으로 채운다는 사실을 드러냈다(실계정으로 확인). 계획서의 "구현 진행 기록" 표에 각 리뷰의 `run_id`와 먼저 실패한 것을 남겼다. Claude 헤드리스 e2e는 D6에 따라 미실행이다.

라이브에서 확인하지 못한 것: 로그아웃 세션의 `FeedList` 형태(A1), 부모 하나의 답글이 10개를 넘을 때의 반환 범위(A11), `naver.me` 단축 링크(A6), 로그인 상태의 차단 서명(A7). 각각 계획서의 가정 절에 남겨 두었고, 코드는 미관측을 근거로 판정을 승격하지 않는다.
...
- 2026-09-07: 승인된 naver-blog 읽기 스킬을 구현했다. 네이버에는 회전하는 토큰이 없어 형제의 registry·refresh·capture 층이 통째로 빠지는 대신, 서버가 자기 총수를 모르는 표면이 많아 종료 판정(정직한 소진·1,000건 상한·페이지 정체)을 CLI 경계가 책임진다. 엔드포인트 장부는 계획서의 스냅샷을 그대로 옮기고 테스트가 두 문서를 직접 대조한다. 글 본문은 유일하게 드리프트하는 표면이라 컴포넌트별 추출 커버리지를 출력에 드러내, 줄어든 본문을 온전한 것처럼 요약할 수 없게 했다. 단계마다 codex 리뷰를 돌려 지적을 재현 테스트와 함께 고쳤다.
