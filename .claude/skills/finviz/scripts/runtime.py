...
def validate_url(url):
    ...
    if parts.scheme != "https" or parts.netloc not in ("finviz.com", "www.finviz.com") or parts.fragment:
        raise Failure(
            "unsupported_url",
            "Only HTTPS Finviz read URLs are supported.",
            "Use a Finviz data URL without credentials, port or fragment.",
        )
    path = parts.path
    pages = (
        "/",
    ...
    allowed |= bool(
        re.fullmatch(
            r"/(?:api/)?calendar/(?:earnings(?:/season-preview)?|dividends|economic(?:/detail/[\w.-]+)?)", path
        )
    )
    allowed |= bool(re.fullmatch(r"/api/stocks-why-moving/by-id/\d+", path))
    allowed |= bool(re.fullmatch(r"/assets/dist(?:-legacy)?/[\w.-]+\.js", path))
    if not allowed:
        raise Failure(
            "unsupported_route",
            "This route is outside the read interface.",
            "Use --help for supported queries; external articles and filings remain links.",
        )
    return url
...
class Store:
    def __init__(self, path):
        ...
        self.db.execute(
            "CREATE TABLE IF NOT EXISTS observations (id TEXT PRIMARY KEY, envelope TEXT NOT NULL, raw BLOB NOT NULL)"
        )
    ...
    def save(self, result, raw):
        with self.db:
            self.db.execute(
                "INSERT INTO observations VALUES (?,?,?)", (result["id"], json.dumps(result, ensure_ascii=False), raw)
            )
    ...
    def get(self, key, raw=False):
        ...
        if not row:
            raise Failure(
                "unknown_result",
                "No saved observation with this ID.",
                "Use the same --store used for the original query.",
            )
...
def fetch(url, args, store, extract):
        ...
        with tempfile.TemporaryDirectory(prefix="finviz-") as tmp:
            ...
            result = dict(
                ...
                source=dict(
                    ...
                    observed_at=now(),
                    http_status=status,
                    received_complete=proc.returncode == 0,
                    headers=fields,
                    redirects=chain.copy(),
                ),
                data=None,
                conditions={
                    k: dict(requested=v[-1], status="unverified", evidence=None)
                    for k, v in parse_qs(urlsplit(origin).query, keep_blank_values=True).items()
                },
                coverage=dict(received=None, shown=None, source_total=None, exhaustive=False),
                continuation=None,
                errors=[],
            )
            try:
                if proc.returncode:
                    raise Failure(
                        "transport",
                        f"curl exited {proc.returncode}: {proc.stderr.decode(errors='replace')[:400]}",
                        "Inspect saved raw bytes; retry as a new observation after resolving connectivity or limits.",
                    )
                if status in (301, 302, 303, 307, 308):
                    ...
                    raise Failure(
                        "access_restricted",
                        f"HTTP {status}; Retry-After={fields.get('retry-after', 'unspecified')}",
                        "Respect Retry-After when present. Do not bypass access controls.",
                    )
                elif status < 200 or status >= 300:
                    raise Failure(
                        "http_error",
                        f"HTTP {status}",
                        "Check the source URL and supported query; the raw response is saved.",
                    )
                else:
                    extract(result, raw)
                ...
                result["errors"].append(
                    ...
                    if isinstance(exc, Failure)
                        ...
                        code="parse_error",
                        message=str(exc),
                        fix="Inspect the saved raw response; the provider structure may have changed.",
                    )
                )
    ...
    raise Failure(
        "redirect_limit",
        "More than five redirects; responses were saved.",
        "Inspect the redirect source; do not follow an unbounded chain.",
    )
