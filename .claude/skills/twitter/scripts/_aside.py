...
def run_snippet(name, args):
    ...
    if not binary:
        raise TwitterError(3, 'Aside is unavailable.', 'Start Aside and make its CLI available on PATH.')
    try:
    ...
    try:
        ...
        raise TwitterError(3, 'Aside returned an invalid response envelope.', 'Run doctor to check the browser bridge.') from None
