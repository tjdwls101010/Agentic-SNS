...
Separate the requested scope, the observed response and the selected output. A populated result can coexist with failed or unattempted targets. Empty returns and null values are observations of missing usable data, not proof of no trading, no holdings or no event; do not replace them with zero. Preserve reported upstream information loss instead of inferring the missing distinction.
