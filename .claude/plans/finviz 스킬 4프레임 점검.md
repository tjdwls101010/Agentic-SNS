...
### 코덱스 독립 리뷰 (`gpt-6-astra` high, `danger-full-access`, 같은 스레드에서 4회)

read-only가 아니라 **네트워크를 허용한** 샌드박스로 돌렸다. 계획의 지적대로 read-only에서는 CLI의 curl이 나가지 못해 픽스처 값을 답으로 내기 때문이고, 그 덕에 네 번 모두 소스 독해가 아니라 **실행 근거**로 판정했다.

| 회차 | 실행 ID | 결과 |
|---|---|---|
| 1차 | `20260919-123105-finviz-reaudit-final-622f` | blocker 1건 + should-fix 6건. **대부분 이번 보완이 만든 회귀다** — 기본 범위를 리프 본문에서 잘라 저장소가 전체를 잃은 것, `--filter`가 창 안에서만 검색한 것, 달력 정렬의 거짓 확정, 복구 문장이 다른 데이터를 준 것 |
| 2차 | `20260919-131637-finviz-reaudit-final-6701` | blocker 해소 확인, should-fix 5건. 그중 하나(`read`의 `coverage.shown`이 갱신되지 않음)는 1차 수정이 만든 회귀 |
| 3차 | `20260919-133828-finviz-reaudit-final-51f9` | 5건 해소 확인, **blocker 없음**. 머지 전 수정 권고 2건(복구 명령의 셸 인용, 봉투 읽기의 coverage 단위) |
| 4차 | `20260919-134854-finviz-reaudit-final-d217` | 최종 확인 |

리뷰가 잡은 결함의 성격이 이번 작업의 교훈이다. **기본 범위를 어디에 두느냐가 저장 계약과 검색 계약을 동시에 건드린다**는 것을 혼자서는 못 봤다. 창을 리프 본문에 두면 (1) 관측이 잘린 채 저장돼 `read`로 이어 읽을 수 없고 (2) `--filter`가 창 안만 본다. 둘 다 "관측은 전부를 보존한다"와 "창은 표시의 문제"라는 계약을 깨는데, 단위 테스트는 창의 *크기*만 보고 있었다. `@leaf window` 선언으로 **저장 이후·`--filter` 이후**에만 적용되게 옮긴 것이 이 두 계약을 동시에 지키는 유일한 배치다.
...
- **오류 문서의 바닥은 155자다.** 코드·메시지·복구 한 줄을 담는 최소 형태이고, 그래서 `--max-chars`는 200 미만을 **받는 자리에서 거절한다** — 담을 수 없는 예산을 받아들이면 그 뒤의 모든 응답이 계약을 어긴다. 목표가 여럿일 때 이 최소 형태는 첫 id만 남기지만, 전체를 다시 받는 `--max-chars` 안내는 정확하다.
- **봉투 최상위 선택은 `--keys`다.** `read ID --fields data`는 이제 `invalid_fields`이고 `--keys data`가 맞다(매핑 계약과 일치). `read ID --limit 1`이 봉투 키를 자르는 것도 같은 계약의 결과다.
- **`read`가 봉투나 `/source` 아래를 읽을 때는 `coverage`를 갱신하지 않는다.** 봉투 키 개수는 행·항목과 다른 단위여서 한 객체에 섞으면 거짓이 된다. 그 개수는 `selection`이 말한다.
